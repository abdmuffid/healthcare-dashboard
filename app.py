...
# Filter function
def create_filters(master_df, sidebar=True):
    ...
    all_specs = ['All Specializations'] + sorted(master_df['specialization'].dropna().unique().tolist())
    filters['specialization'] = st.sidebar.multiselect(
    ...
    # Status filter
    all_statuses = ['All Status'] + sorted(master_df['status'].unique().tolist())
    ...
    # Gender filter
    all_genders = ['All Genders'] + sorted(master_df['gender'].unique().tolist())
    ...
    all_insurance = ['All Insurance'] + sorted(master_df['insurance_provider'].dropna().unique().tolist())
    filters['insurance'] = st.sidebar.multiselect(
        "Insurance Provider",
    ...
    min_age, max_age = st.sidebar.slider(
        "Patient Age Range",
        min_value=int(master_df['age'].min()),
        max_value=int(master_df['age'].max()),
        value=(int(master_df['age'].min()), int(master_df['age'].max()))
    )
    ...
    min_cost, max_cost = st.sidebar.slider(
        ...
        min_value=int(master_df['cost'].min()),
        max_value=int(master_df['cost'].max()),
        value=(int(master_df['cost'].min()), int(master_df['cost'].max()))
    )
    filters['cost_range'] = (min_cost, max_cost)
...
def apply_filters(df, filters):
    ...
    # Specialization filter
    if 'All Specializations' not in filters['specialization'] and filters['specialization']:
        filtered_df = filtered_df[filtered_df['specialization'].isin(filters['specialization'])]
    
    # Status filter
    if 'All Status' not in filters['status'] and filters['status']:
        filtered_df = filtered_df[filtered_df['status'].isin(filters['status'])]
    
    # Gender filter
    if 'All Genders' not in filters['gender'] and filters['gender']:
        filtered_df = filtered_df[filtered_df['gender'].isin(filters['gender'])]
    
    # Insurance filter
    if 'All Insurance' not in filters['insurance'] and filters['insurance']:
        filtered_df = filtered_df[filtered_df['insurance_provider'].isin(filters['insurance'])]
    ...
    # Cost range filter
    filtered_df = filtered_df[
        (filtered_df['cost'] >= filters['cost_range'][0]) & 
        (filtered_df['cost'] <= filters['cost_range'][1])
    ]
...
def calculate_metrics(filtered_df):
    ...
    total_revenue = filtered_df['cost'].sum()
    completed_revenue = filtered_df[filtered_df['status'] == 'Completed']['cost'].sum()
    lost_revenue = total_revenue - completed_revenue
    no_show_rate = filtered_df['is_no_show'].mean()
    completion_rate = (filtered_df['status'] == 'Completed').mean()
    ...
    return {
        ...
        'completed_revenue': completed_revenue,
        'lost_revenue': lost_revenue,
        'avg_patient_age': filtered_df['age'].mean()
    }
...
def main():
    ...
    # Create filters
    filters = create_filters(master_df)
    ...
    if len(filtered_df) < len(master_df):
        st.info(f"📋 Filters Applied: Showing {len(filtered_df)} of {len(master_df)} appointments")
    ...
    if page == "Executive Dashboard":
        executive_dashboard(filtered_df, filtered_metrics)
    elif page == "Operations Analytics":
        operations_analytics(filtered_df)
    elif page == "Financial Analytics":
        financial_analytics(filtered_df, filtered_metrics)
    elif page == "Quality Analytics":
        quality_analytics(filtered_df)
    elif page == "Business Recommendations":
        business_recommendations(filtered_df, filtered_metrics)
...
def executive_dashboard(master_df, metrics):
    ...
    with col1:
        st.metric(
            ...
            value=f"{int(metrics['total_appointments'])}",
            delta=None
        )
    ...
    with col3:
        ...
        **Growth Opportunities**
        - Appointments: {int(metrics['total_appointments'])}
...
def financial_analytics(master_df, metrics):
    ...
    with col2:
        if current_no_show > 0:
            ...
            roi_percentage = (potential_recovery - implementation_cost) / implementation_cost * 100 if implementation_cost > 0 else 0
            
            st.metric("Potential Annual Recovery", f"${potential_recovery:,.0f}")
            st.metric("ROI", f"{roi_percentage:.0f}%")
            st.metric("Payback Period", f"{implementation_cost/potential_recovery*12:.1f} months" if potential_recovery > 0 else "N/A")
...
def business_recommendations(master_df, metrics):
    ...
    st.error(f"""
    **CRITICAL ISSUE:** Current no-show rate of {no_show_rate:.1f}% is causing ${lost_revenue:,.0f} in lost revenue.
        ...
        if len(spec_analysis) > 0:
            ...
            with col1:
                ...
                **Current State Analysis:**
                - Highest volume: {highest_volume} ({spec_analysis.loc[highest_volume, 'appointments']:.0f} appointments)
